...
- MIT的Akarsh Kumar和Phillip Isola提出**Supervised Memory Training（SMT）**，将非线性RNN训练转化为对单步记忆转移标签的监督学习。先用Transformer编码器在预测状态目标上训练获取记忆标签（保留预测未来所需的过去信息），然后RNN仅做单步监督训练，任意两个token间梯度路径长度为**O(1)**，完全无需展开RNN。在语言建模和像素序列建模任务上，SMT**优于BPTT基线**，使非线性RNN能更好地捕获长程依赖。该方法将"记住什么"与"如何更新记忆"解耦，有望解锁构建时间抽象的RNN模型规模化。
