...
from config_loader import (
    non_news_keywords, compute_keywords, capex_keywords,
    research_keywords, funding_keywords, model_keywords,
    tier1_ai_companies
)
...
def fix_category(title: str, summary: str = "", current_cat: str = "") -> str:
    ...
    # 初创&融资：融资、投资、收购、上市（排除capex/基础设施投资）
    if any(k in text for k in capex_keywords()):
        pass  # capex类投资归算力追踪，跳过融资判断
    elif any(k in text for k in funding_keywords()):
        return "初创&融资"

    # 模型前沿：NVIDIA发布的模型（如Nemotron）应该归模型前沿
    if "nvidia" in text and any(k in text for k in ["模型", "model", "参数", "开源", "open", "nemotron"]):
        return "模型前沿"

    # 算力追踪：硬件、芯片、云服务、算力供需
    if any(k in text for k in compute_keywords()) and not any(k in text for k in ["sue", "诉讼", " tariffs"]):
        return "算力追踪"
    ...
    # 模型前沿：VLA、世界模型发布
    if any(k in text for k in ["vla", "world model", "世界模型"]) and any(k in text for k in ["发布", "release", "launch", "开源", "open"]):
        return "模型前沿"

    # 机器人：区分研究和产品落地
    if any(k in text for k in ["robot", "机器人", "机械爪"]):
        # 学术研究 -> 研究关注
        if any(k in text for k in ["论文", "paper", "arxiv", "研究", "research", "实验室", "龙虾"]):
            return "研究关注"
        # 产品落地/试运行 -> 产业动态
        return "产业动态"
...
def normalize_for_comparison(text: str) -> set:
    ...
    # 英文分词
    english_words = set(w for w in re_module.findall(r'[a-z]+', text) if w not in stopwords and len(w) > 1)
    # 中文字符2-gram（解决单字符太碎片化问题）
    chinese_text = re_module.sub(r'[a-z0-9\s]', '', text)
...
def improve_news(articles: List[Dict], do_filter: bool = True) -> List[Dict]:
    ...
    # 5. 标题相似度去重（处理同一事件不同来源）
    improved = filter_similar_duplicates(improved, threshold=0.35, debug=True)
    after_simi = len(improved)
    if do_filter:
        print(f"   相似去重后: {after_simi} 条")
    else:
        print(f"   [后规范化] 相似去重后: {after_simi} 条")
