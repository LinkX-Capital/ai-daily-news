...
- 模型前沿：字节跳动Seed团队发布Seedance 2.5，单次生成30秒视频，支持多模态参考与精准编辑
- 产业动态：Perplexity将Spaces升级为Projects，共享文件系统+持久记忆
- 研究关注：Explorative Modeling，提出预训练"第三轴"探索，6.2×数据效率与1.43 FID; 腾讯Hyra研究代理辅助解决50年数学难题，和集与差集最优指数确认为2; ALIGN，自动生成对齐接口修复Agent-环境失配，ALFWorld成功率提升45.67%; CG-World，85万段CG对齐数据集，带反事实分支面向世界模型
- X讨论：追求预训练数据质量的尽头是自建私有搜索引擎，Anthropic新增TurboPuffer暗示自建索引
...
- Seedance 2.5延续2.0的多模态音视频联合生成架构，重点突破长叙事、多模态参考和编辑能力。**单次生成时长从15秒提升至30秒**，支持多轮延长以生成数分钟连贯内容，优化了镜头衔接和场景过渡。多模态参考支持单次输入**最多30张图片、10段视频、10段音频**，并提升**白模参考**、运动参考、创意参考能力；编辑方面支持时间戳精准控制、绿幕编辑、视角编辑等。模型系统优化了"油腻感"问题，并已进入**教育**（豆包爱学「豆包课堂」）、**工业制造**（合成数据训练机器人）及**自动驾驶**（模拟极端天气与复杂路况）等产业场景。已在即梦AI、豆包专业版上线，API将近期上线火山方舟。
...
- OpenBMB团队提出ALIGN框架，解决LLM Agent与环境之间的接口失配问题--Agent失败往往不是推理错误，而是对动作效果的预期与环境实际状态转移不一致。ALIGN通过INFERRULES模块提取环境静态规则、WRAPSTEP模块增强逐步观察，作为轻量级Python包装器运行，不修改Agent逻辑或环境代码。在ALFWorld上仅优化接口反馈就将Qwen2.5-7B从**13.4%提升至31.3%**，四个基准测试中最高提升**45.67%**，连续无效动作减少65%。接口可跨Agent架构（ReAct、Self-Consistency、Planning）和LLM骨干即插即用迁移，无需重新生成。
