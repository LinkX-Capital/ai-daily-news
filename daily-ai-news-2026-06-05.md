...
**Audio Interaction Model：统一在线音频交互模型，实时感知-决策-响应**
- 论文提出Audio Interaction Model（AIM），将离线音频语言模型统一为在线流式模型，通过始终在线的感知-决策-响应循环，实时监听声音、环境和指令并即时反应。研究提出SoundFlow框架，涵盖流式数据构建、comprehension-aware training和异步低延迟推理。配套构建了**260万条**流式语料库StreamAudio-2M（覆盖7项基础能力和28个子任务），以及Proactive-Sound-Bench评测基准。在8个benchmark上，模型在保持主流音频任务竞争力的同时，解锁了实时ASR、流式音频指令跟随和主动干预等离线模型无法实现的能力。
  > 💡 音频交互从离线走向在线流式是语音助手演进的必然方向，感知-决策-响应统一框架为实时多模态交互奠定了基础。
   - 来源: [arXiv](https://arxiv.org/abs/2606.05121)
