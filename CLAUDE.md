...
触发条件（写完 body 后自检）：
- 缺少关键数据（参数量、benchmark、金额、时间线）
- 只有 slogan 级描述，无法回答"具体做了什么"
- 标题承诺的信息 body 没有兑现

按来源类型分层搜索：
1. **模型/产品发布** → 官方 blog/docs → GitHub repo README → HuggingFace model card → 官方推文线程
2. **论文** → arXiv abstract → paper PDF intro/results → 作者推文解读
3. **融资/商业** → The Information/Bloomberg 原文 → Crunchbase → 公司官网 press release
4. **X 推文类** → 推文线程完整展开 → 关联的官方链接 → 同一账号近期相关推文

搜索纪律：
- **有明确来源就不搜**：已有官方链接或用户补充的链接时，直接读该链接即可，不额外搜索。只在链接不可达或读完后仍缺关键数据时才启动搜索
- 搜到的信息必须可追溯到具体来源
- 最多 2 轮搜索，避免无限兔子洞
- 搜不到就降级，不猜测
...
- **来源不可达时，不补充额外句子凑数**：宁可2句，也不用AI推理/常识/猜测来补第3句
...
**要点速览 rules:**
- 直接使用“详细参考”中的条目标题作为要点速览标题，不重新概括、不取冒号前半句、不截断标题
- 例如：「FlashAttention-4发布：H100上前向速度提升22%」「Claude消费者增长加速：移动端使用占比提升」
