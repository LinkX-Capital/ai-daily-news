...
- 产业动态：Anthropic发布双重企业功能：Claude Code Artifacts实时协作 + MCP集中授权管理; OpenAI Codex新增Record & Replay功能将macOS工作流转化为可复用技能; 月之暗面Kimi Work上线Goal Mode支持桌面Agent长任务持续执行; xAI的Grok模型接入Databricks Agent Bricks，面向企业Agent场景; Snap因成本压力剥离AI视频团队成立独立公司Dotmo
...
- OpenAI发布Codex app 26.616，新增Record & Replay功能（macOS），可将用户演示的工作流程自动转化为可复用的技能（skill）。该功能首次推出时不包括欧洲经济区、英国和瑞士，需启用Computer Use权限。更新还包括：自动化运行历史批量操作、本地与远程主机间的线程切换（thread handoff）、改进的Browser Use持久化等。CLI版本0.141.0引入认证的端到端加密Noise中继通道用于远程执行器，并支持跨平台远程执行时保留原生工作目录和shell环境。
  > 💡 Record & Replay把"演示即编程"推向实用化，降低了自动化技能创建的门槛，与Anthropic Computer Use和Kimi Goal Mode形成直接竞争。
   - 来源: [OpenAI Codex Changelog](https://developers.openai.com/codex/changelog#codex-2026-06-18-app)
